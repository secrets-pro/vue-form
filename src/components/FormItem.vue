...
export default {
  ...
  methods: {
              ...
              ...modelKeysSorted.map((el) => {
                ...
                return h("vue-form-item", {
                  ...
                  class: [
                    ...
                    modelKeysSorted.length >= 3 ? "maxWidth33" : "normal-vue-item",
                  ],
                  on: {
                    "on-copy": (value) => {
    ...
    renderArray(h, config, prop, model) {
      ...
      //  判断是否是一级属性
      let title = extraOptions(config.description).title || config.title || prop;
    ...
    renderFun(h, config, prop, currentValue, _arrayIndex, slot) {
      ...
      let props = {
        ...
        value: _arrayIndex !== undefined ? currentValue[_arrayIndex] : currentValue,
      };
      // ui 配置
      ...
      if (type === "radio" || type === "checkbox") {
        type = `${type}-group`;
        children = renderRadioCheckbox(h, config.type, config.options, this.prefix);
      } else if (type === "date") {
        type = "date-picker";
        ...
        children = renderSelectOptions(h, config.options, config.groups, this.prefix);
        type = "select";
        // style.width = "100%";
      ...
      if (extra.url) {
        ...
        if (data) {
          ...
          try {
            ...
            if (res && res.data) {
              data = res.data;
              if (this.config.name == "quotaModel" && !res.data.includes(this.value)) {
                try {
                  let p = JSON.parse(this.value);
                  data.push({
                    jsonStr: this.value,
                    name: `自定义（cpu[${p.limitCpu}]内存[${p.requestMemory}]）`,
                  });
                } catch (error) {}
              }
        ...
        if (muti && __enum__.length) {
          ...
          if (
            __current_value__ &&
            __enum__.length &&
            !__enum__.includes(__current_value__)
          ) {
            __current_value__ = "";
            this.currentValue = "";
            this.$emit("input", "");
